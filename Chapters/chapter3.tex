...
Signaling interactions were obtained using the \texttt{import\_omnipath\_interactions} function and with assigned mechanism = 0, whereas transcriptional regulatory interactions imported using \texttt{import\_transcriptional\_ interactions} were annotated as mechanism = 1. 
...
The default parameters were also used to run the algorithms, given the complexity of the study. However, the wrapper function if prepared to accept custom parameters for each algorithm, if needed, including for converting to \gls{DEGs} using fold change and p-value thresholds.
